(function(){
    ...
    //Group all books by language and sort them by author (and price)
    var sorted = _.sortByAll(array, ['author', 'price']);
    console.log(_.groupBy(sorted, function(book){
        return book.language;
    }));

    //Get average book price for each author


    //Get all books in English or German with price below 30 and group by author
    var filtered = _.filter(_.filter(array, function(book){
        return (book.language === "English" || book.language === "German");
    }), function(book) {
        return parseFloat(book.price) < 30;
    });

    console.log(_.groupBy(filtered, function(book) {
        return book.author;
    }));
}());